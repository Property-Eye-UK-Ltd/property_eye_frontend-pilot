...
const Reports = () => {
  ...
  const verifyMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(
        `Verification complete: ${data.confirmed_fraud} confirmed fraud, ${data.not_fraud} cleared.`
      );
  ...
  const handleVerifyAllHighConfidence = () => {
    ...
    const highConfidenceIds = reports
      .filter(
        (r) =>
          r.confidence_score >= 85 && r.verification_status === "suspicious"
      )
      .map((r) => r.id);
    ...
    if (
      confirm(
        `Verify ${highConfidenceIds.length} high confidence matches? This will check Land Registry records.`
      )
    ) {
